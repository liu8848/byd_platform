...
public class DepartmentConvertor implements Converter<Long> {
    @Override
    ...
    public Long convertToJavaData(ReadConverterContext<?> context) throws Exception {
        return DictUtil.getDictValue(DictKeyConstant.DEPARTMENT,context.getReadCellData().getStringValue());
    }

    @Override
    public WriteCellData<?> convertToExcelData(WriteConverterContext<Long> context) throws Exception {
        return new WriteCellData<>(DictUtil.getDictName(DictKeyConstant.DEPARTMENT,context.getValue()));
    }
}
