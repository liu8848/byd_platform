...
public class AuditorLevelConvertor implements Converter<Long> {
    @Override
    ...
    public Long convertToJavaData(ReadConverterContext<?> context) throws Exception {
        return DictUtil.getDictValue(DictKeyConstant.AUDITOR_LEVEL,context.getReadCellData().getStringValue());
    }

    @Override
    public WriteCellData<?> convertToExcelData(WriteConverterContext<Long> context) throws Exception {
        return new WriteCellData<>(DictUtil.getDictName(DictKeyConstant.AUDITOR_LEVEL,context.getValue()));
    }
}
